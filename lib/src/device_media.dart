...
class DeviceMedia {
  static const MethodChannel _channel = const MethodChannel('device_media');
  ...
  static Future<List<ImageFolder>> getFoldersImages() async {
    final List<dynamic> result =
        await _channel.invokeMethod('getFoldersImages');
    final List<ImageFolder> imageFolders =
        List<ImageFolder>.from(result.map((e) => ImageFolder.fromMap(e)));
    return imageFolders;
  }
  ...
  static Future<List<DeviceVideo>> getVideos() async {
    ...
    final List<DeviceVideo> videos =
        List<DeviceVideo>.from(result.map((e) => DeviceVideo.fromMap(e)));
    return videos;
  }
}
